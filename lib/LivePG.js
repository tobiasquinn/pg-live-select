...
LivePG.prototype._init = (function () {
  ...
  var performNextUpdate = (function () {
    ...
    if (this.waitingToUpdate.length !== 0) {
      (function () {
        var queriesToUpdate = _.uniq(_this2.waitingToUpdate.splice(0, _this2.waitingToUpdate.length));

        var resolvedCount = 0;
        var resolvedHandler = function resolvedHandler() {
          ++resolvedCount === queriesToUpdate.length && performNextUpdate();
        };

        queriesToUpdate.forEach(function (queryHash) {
          return _this2._updateQuery(queryHash, resolvedHandler);
        });
      })();
    } else {
    ...
    client.query('\n      WITH\n        res AS (' + queryBuffer.query + '),\n        data AS (\n          SELECT\n            res.*,\n            MD5(CAST(ROW_TO_JSON(res.*) AS TEXT)) AS _hash,\n            ROW_NUMBER() OVER () AS _index\n          FROM res),\n        data2 AS (\n          SELECT\n            1 AS _added,\n            data.*\n          FROM data\n          WHERE _hash NOT IN (\'' + oldHashes.join('\',\'') + '\'))\n      SELECT\n        data2.*,\n        data._hash AS _hash\n      FROM data\n      LEFT JOIN data2\n        ON (data._index = data2._index)', queryBuffer.params, function (error, result) {
      ...
      try {
        for (var _iterator4 = _getIterator(queryBuffer.handlers), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
          ...
          updateHandler.emit('update', filterHashProperties(diff), filterHashProperties(newData));
        }
      } catch (err) {
