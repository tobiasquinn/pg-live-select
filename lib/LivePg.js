...
function LivePg(settings, options) {
  var self = this;
  LiveSelectBase.call(self, options);

  self.connStr = settings.connStr;
  self.channel = settings.channel || randomString.alphaLower(6);
  self.triggerFun = 'livepg_' + self.channel;
...
LivePg.prototype._initSelect = function(queryHash, buffer, callback) {
  var self = this;
  ...
  if(queryHash in self.tablesUsedCache) {
    ...
    findDependentRelations(self.connStr, buffer.query, buffer.params,
  ...
  pg.connect(self.connStr, function(error, client, done) {
    ...
    client.query(
      ...
      function(error, result) {
        ...
        callback &&
          callback(null, matchUpdateResult(queryBuffer.data, result.rows));
      }
    );
  });

  var matchUpdateResult = function(origin, result) {
    var existingRows = {};
    origin.forEach(function(row) {
      existingRows[row._hash] = row;
    });

    return result.map(function(row, index) {
      if(row._added === null) {
        row = existingRows[row._hash];
      }
