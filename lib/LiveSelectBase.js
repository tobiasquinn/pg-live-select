...
function LiveSelectBase(options) {
  ...
  self.options.checkConditionWhenQueued =
    self.options.checkConditionWhenQueued || false;
...
LiveSelectBase.prototype._matchRowEvent = function(table, row, newRow) {
  ...
  if(table in self.allTablesUsed) {
    self.allTablesUsed[table].forEach(function(queryHash) {
      ...
      // If the query is currently updating and the configuration allows,
      //  skip checking if necessary to update this query again
      if(queryBuffer.updating === true &&
          self.options.checkConditionWhenQueued === false) return;
...
/*
  ...
  @param callback Function (error, result) Result is new result set
  @return undefined
*/
