...
exports.apply = function(data, diff) {
  ...
  // copy the removed data and return as seperate result
  _.forEach(diff.removed, function(r) {
      // copy all removed
      removedResults.push(newResults[r._index - 1]);
  });
  // remove records found in the moved list from the removeResults
  var movedIndexes = _.map(diff.moved, function (m) { return m.old_index; });
  removedResults = _.filter(removedResults, function(r) {
      return _.indexOf(movedIndexes, r._index) == -1;
  });
